...
public partial class Tutorial : Control
{
    ...
    {
        if (_panelContainer != null && !_panelContainer.Visible)
        {
            _panelContainer.Visible = true;
            _currentStepIndex = 0;
            ShowCurrentStep();
            GetTree().Paused = true; // Spiel pausieren

            // Video und Button vom Pause-Modus ausnehmen
            ...
            if (_nextButton != null)
            {
                _nextButton.ProcessMode = ProcessModeEnum.Always;
            }

            _panelContainer.GrabFocus();
        }
    }
